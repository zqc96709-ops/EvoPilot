...
function App() {
  ...
  const sendAi = async (preset?: string, contextOverride: Partial<AgentContext> = {}) => {
    ...
    const requestContext = { ...agentContext, ...contextOverride, recentConversation: nextChat.slice(-10).map(({ role, content }) => ({ role, content })) }
    setAiBusy(true)
...
function ProjectWorkspace({ project, records, onBack, onOpen, onCreate, onEdit, onStartTimer, onAiAnalyze }: { project: RecordData; records: RecordData[]; onBack: () => void; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onEdit: (record: RecordData) => void; onStartTimer: (record: RecordData) => void; onAiAnalyze: (question: string, context: Partial<AgentContext>) => void }) {
  ...
  const map: { key: typeof tab; label: string; entity?: Entity }[] = [{ key: 'overview', label: '概览' }, { key: 'tasks', label: '任务', entity: 'tasks' }, { key: 'hypotheses', label: '假设', entity: 'hypotheses' }, { key: 'experiments', label: '实验', entity: 'experiments' }, { key: 'results', label: '结果', entity: 'results' }, { key: 'financialTransactions', label: '财务', entity: 'financialTransactions' }, { key: 'timeLogs', label: '时间', entity: 'timeLogs' }, { key: 'decisions', label: '决策', entity: 'decisions' }, { key: 'reviews', label: '复盘', entity: 'reviews' }, { key: 'timeline', label: '时间线' }]
...
function OutcomesView({ records, onOpen, onCreate }: { records: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void }) {
  ...
  const achieved = outcomes.filter((record) => record.status === 'ACHIEVED').length
  const verified = outcomes.filter((record) => record.evidenceStatus === 'VERIFIED').length
  return <div className="outcomes-page"><section className="section-intro"><span>✓</span><div><h2>Outcome / Result</h2><p>结果不是完成任务，而是比较当时预期、现实结果、偏差与证据。</p></div><button className="button primary" onClick={() => onCreate('results', { status: 'PLANNED', outcomeType: 'QUANTITATIVE', metricKind: 'NUMBER', evidenceStatus: 'RECORDED', currency: 'CNY', date: today() })}>＋ 记录 Outcome</button></section><div className="metric-strip four"><Metric label="全部 Outcome" value={`${outcomes.length}`} /><Metric label="已达成" value={`${achieved}`} /><Metric label="已核验" value={`${verified}`} /><Metric label="数据缺口" value={`${outcomes.filter((record) => record.evidenceStatus === 'MISSING').length}`} /></div>{outcomes.length ? <div className="outcome-grid">{outcomes.map((outcome) => <article key={outcome.id} onClick={() => onOpen(outcome)}><header><span>{String(outcome.outcomeType || 'QUALITATIVE')}</span><b>{statusLabel(outcome.status)}</b></header><h3>{titleFor(outcome)}</h3><p>{descriptionFor(outcome) || String(outcome.actual || outcome.expected || '尚未记录实际结果。')}</p><div><span>预期<strong>{outcome.metricKind === 'MONEY' ? outcome.targetAmountMinor ? formatMoneyMinor(String(outcome.targetAmountMinor), String(outcome.currency || 'CNY')) : '未记录' : String(outcome.targetValue || outcome.expected || '未记录')}</strong></span><span>实际<strong>{outcome.actualAmountMinor || outcome.actualValue || outcome.actual ? outcome.metricKind === 'MONEY' ? formatMoneyMinor(String(outcome.actualAmountMinor || '0'), String(outcome.currency || 'CNY')) : String(outcome.actualValue || outcome.actual) : '未记录'}</strong></span><span>证据<strong>{statusLabel(outcome.evidenceStatus)}</strong></span></div>{Boolean(outcome.achievementBps) && <footer>完成率 {(Number(outcome.achievementBps) / 100).toFixed(2)}%</footer>}</article>)}</div> : <GuidedEmpty icon="✓" title="还没有 Outcome" text="先为一个重要项目记录预期结果；执行后再填写实际、偏差与证据。" action="创建第一个 Outcome" onAction={() => onCreate('results', { status: 'PLANNED', evidenceStatus: 'RECORDED', currency: 'CNY', date: today() })} />}</div>
}
...
function SearchOverlay({ query, results, selectedEntities, onToggleEntity, onOpen, onClose }: { query: string; results: RecordData[]; selectedEntities: Entity[]; onToggleEntity: (entity: Entity) => void; onOpen: (record: RecordData) => void; onClose: () => void }) {
  const filters = entities.filter((config) => ['goals', 'projects', 'tasks', 'timeLogs', 'events', 'people', 'knowledge', 'notes', 'hypotheses', 'experiments', 'results', 'reviews', 'insights', 'principles', 'mentalModels', 'decisions'].includes(config.entity))
...
function RecordDrawer({ record, records, onClose, onEdit, onArchive, onCreate, onOpen, onStartTimer, onAddModel }: { record?: RecordData; records: RecordData[]; onClose: () => void; onEdit: (record: RecordData) => void; onArchive: (id: string) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onOpen: (record: RecordData) => void; onStartTimer: (record: RecordData) => void; onAddModel: (id: string) => void }) {
  ...
  const conversions = record.entity === 'notes' ? ['knowledge', 'insights', 'mentalModels', 'decisions', 'tasks', 'projects'] as Entity[] : record.entity === 'reviews' ? ['insights', 'knowledge', 'principles', 'mentalModels', 'decisions', 'tasks'] as Entity[] : record.entity === 'results' ? ['reviews'] as Entity[] : record.entity === 'inbox' ? ['tasks', 'knowledge', 'insights', 'hypotheses', 'decisions', 'events', 'projects'] as Entity[] : []
...
function RelationshipOverview({ record, records, onOpen }: { record: RecordData; records: RecordData[]; onOpen: (record: RecordData) => void }) {
  ...
  const order: Entity[] = record.entity === 'goals' ? ['projects', 'tasks', 'timeLogs', 'results', 'reviews', 'decisions', 'insights'] : record.entity === 'projects' ? ['tasks', 'timeLogs', 'financialTransactions', 'results', 'reviews', 'decisions', 'knowledge', 'people', 'events'] : ['timeLogs', 'results', 'reviews', 'knowledge', 'decisions']
