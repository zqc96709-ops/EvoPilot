...
import {
  ...
  percent, priorityLabel, recordDate, statusLabel, titleFor,
...
const defaultStatus = (entity: Entity) => ({ tasks: 'todo', goals: 'active', projects: 'active', hypotheses: 'untested', experiments: 'planned', decisions: 'pending', inbox: 'unprocessed', notes: 'INBOX', notebookFiles: 'ACTIVE', results: 'PLANNED', deliverables: 'DRAFT', resultPackages: 'ACTIVE', capabilityPacks: 'DRAFT', capabilityAssets: 'DRAFT', capabilityAssetVersions: 'DRAFT', capabilityPackItems: 'ACTIVE', capabilityWorkspaceFolders: 'ACTIVE', capabilityWorkspaceEntries: 'ACTIVE', packApplications: 'APPLIED', packApplicationItems: 'ACTIVE', templateInstances: 'ACTIVE', capabilityImprovementProposals: 'DRAFT', workflows: 'ACTIVE', workflowVersions: 'EXPERIMENTAL', workflowRuns: 'PLANNED', workflowRunSteps: 'PLANNED', workflowImprovementProposals: 'DRAFT', financialAccounts: 'ACTIVE', financialCategories: 'ACTIVE', financialTransactions: 'POSTED' } as Partial<Record<Entity, string>>)[entity] || 'active'
...
function App() {
  ...
  const confirmAiAction = async (actionId: string, baseChat = chat) => {
    ...
    try {
      ...
      setChat([...updated, { role: 'assistant', content: result.duplicate ? `相同的${label}操作已经完成，没有重复写入。` : `✓ 已保存${label}，${PRODUCT_NAME} 已同步更新。` }])
  ...
  return <div className={`app-shell ${view === 'today' ? 'today-route' : ''} ${view === 'finance' ? 'finance-route' : ''} ${view === 'time' ? 'time-route' : ''} ${activeTheme === 'light' ? 'light-theme' : 'dark-theme'} ${effectiveSidebarOpen ? "" : "sidebar-collapsed"} ${sidebarPeek ? "sidebar-peek" : ""} ${aiOpen ? "ai-open" : ""} ${aiResizing ? "ai-resizing" : ""}`} style={{ "--ai-width": `${aiWidth}px` } as React.CSSProperties} onPointerMove={handleShellPointer}>
    ...
    <main className="main-content">
      ...
      {view === 'projects' && <ProjectsView records={records} selectedId={selectedProjectId} onSelect={setSelectedProjectId} onOpen={openRecord} onCreate={openCreate} onArchive={archiveRecord} onSave={async (entity, data) => { await api.save(entity, data); await refresh(); showNotice('已保存到本机。') }} onApplyCapabilityPack={applyCapabilityPack} onAiAnalyze={(question, context) => { setAiOpen(true); void sendAi(question, context) }} />}
      {view === 'outcomes' && <OutcomesView records={records} onOpen={openRecord} onCreate={openCreate} onApplyCapabilityPack={applyCapabilityPack} onPublishCapabilityVersion={publishCapabilityVersion} onRefresh={refresh} />}
...
function ProjectsView({ records, selectedId, onSelect, onOpen, onCreate, onArchive, onSave, onApplyCapabilityPack, onAiAnalyze }: { records: RecordData[]; selectedId: string | null; onSelect: (id: string | null) => void; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void>; onApplyCapabilityPack: (pack: RecordData, projectId: string) => Promise<void>; onAiAnalyze: (question: string, context: Partial<AgentContext>) => void }) {
  const projects = records.filter((record) => record.entity === 'projects')
  const selected = projects.find((project) => project.id === selectedId)
  if (selected) return <ProjectWorkspace project={selected} records={records} onBack={() => onSelect(null)} onOpen={onOpen} onCreate={onCreate} onArchive={onArchive} onSave={onSave} onApplyCapabilityPack={onApplyCapabilityPack} onAiAnalyze={onAiAnalyze} />
  return <ProjectIntelligenceView records={records} onOpen={onOpen} onSelectProject={onSelect} onCreate={onCreate} />
}

type ProjectWorkbenchTab = 'overview' | 'execution' | 'data' | 'materials' | 'review'

function ProjectWorkspace({ project, records, onBack, onOpen, onCreate, onArchive, onSave, onApplyCapabilityPack, onAiAnalyze }: { project: RecordData; records: RecordData[]; onBack: () => void; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void>; onApplyCapabilityPack: (pack: RecordData, projectId: string) => Promise<void>; onAiAnalyze: (question: string, context: Partial<AgentContext>) => void }) {
  const [tab, setTab] = useState<ProjectWorkbenchTab>('overview')
  const [quickCaptureOpen, setQuickCaptureOpen] = useState(false)
  const [capabilityPickerOpen, setCapabilityPickerOpen] = useState(false)
  const related = records.filter((record) => linkedTo(record, project.id))
  const tasks = related.filter((record) => record.entity === 'tasks')
  const runs = related.filter((record) => record.entity === 'workflowRuns')
  const logs = related.filter((record) => record.entity === 'operationalLogs')
  const tables = related.filter((record) => record.entity === 'templateInstances' && record.instanceKind === 'TABLE')
  const materials = projectMaterials(records, project.id)
  const reviews = related.filter((record) => record.entity === 'reviews')
  const results = related.filter((record) => record.entity === 'results')
  const economics = projectEconomics(records, project.id)
  const time = related.filter((record) => record.entity === 'timeLogs').reduce((sum, log) => sum + durationMinutes(log), 0)
  const tabs: { key: ProjectWorkbenchTab; label: string; count?: number }[] = [
    { key: 'overview', label: '总览' }, { key: 'execution', label: '执行', count: tasks.filter(isActive).length }, { key: 'data', label: '数据', count: tables.length }, { key: 'materials', label: '资料', count: materials.length }, { key: 'review', label: '复盘', count: reviews.length },
  ]
  const activeRuns = runs.filter((item) => ['RUNNING', 'PLANNED', 'PAUSED'].includes(String(item.status)))
  const attentionLogs = logs.filter((item) => ['ISSUE', 'HYPOTHESIS'].includes(String(item.type)))
  const recentChanges = logs.filter((item) => ['ACTION', 'DECISION'].includes(String(item.type)))
  return <div className="project-workspace project-workbench"><button className="back-link" onClick={onBack}>← 返回项目</button><section className="workspace-hero"><div><div className="meta-line"><span className={`health ${project.health || 'healthy'}`} />{String(project.stage || statusLabel(project.status))} · {String(project.category || '未设项目类型')} · {relationName(project.goalId, records) || '未关联目标'}</div><h2>{titleFor(project)}</h2><p>{String(project.why || project.description || '尚未填写项目为什么值得投入。')}</p></div><div className="project-workbench-actions"><button className="button" onClick={() => setQuickCaptureOpen(true)}>快速记录</button><button className="button" onClick={() => setCapabilityPickerOpen(true)}>调用能力</button><button className="button" onClick={() => onAiAnalyze(`基于项目「${titleFor(project)}」的现有任务、运行、执行记录与结果，指出最应优先处理的一项问题。`, { currentProjectId: project.id, currentGoalId: String(project.goalId || '') || undefined })}>AI</button><button className="button" onClick={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })}>＋ 创建任务</button><button className="button primary" onClick={() => onCreate('workflowRuns', { title: `${titleFor(project)} · 工作链运行`, projectId: project.id, goalId: project.goalId, status: 'PLANNED', startedAt: nowInput() })}>▶ 开始工作流</button></div></section><nav className="workspace-tabs">{tabs.map((item) => <button key={item.key} className={tab === item.key ? 'active' : ''} onClick={() => setTab(item.key)}>{item.label}{item.count !== undefined && <span>{item.count}</span>}</button>)}</nav>
    {quickCaptureOpen && <ProjectWorkbenchDialog title="快速记录" onClose={() => setQuickCaptureOpen(false)}><OperationalQuickCapture project={project} records={records} onSave={onSave} onSaved={() => setQuickCaptureOpen(false)} /></ProjectWorkbenchDialog>}
    {capabilityPickerOpen && <ProjectWorkbenchDialog title="调用能力" onClose={() => setCapabilityPickerOpen(false)}><ProjectCapabilityPicker project={project} records={records} onApply={onApplyCapabilityPack} onClose={() => setCapabilityPickerOpen(false)} /></ProjectWorkbenchDialog>}
    {tab === 'overview' && <div className="workspace-overview project-workbench-overview"><div className="metric-strip"><Metric label="项目进度" value={`${percent(project.progress)}%`} /><Metric label="实际投入" value={formatMinutes(time)} /><Metric label="当前运行" value={activeRuns.length ? `${activeRuns.length} 项` : '未开始'} /><Metric label="真实结果" value={results.length ? `${results.length} 条` : '未记录'} hint={economics.postedTransactions ? formatMoneyMinor(economics.managementContributionMinor) : '不以估算替代事实'} /></div><ProjectOperatingState project={project} onSave={onSave} /><div className="three-column"><section className="work-panel"><PanelHeader title="今天可推进的任务" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />{tasks.filter(isActive).length ? tasks.filter(isActive).slice(0, 5).map((task) => <CompactRecord key={task.id} record={task} onOpen={onOpen} />) : <GuidedEmpty icon="□" title="还没有执行任务" text="任务是下一步行动；不创建第二套任务系统。" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />}</section><section className="work-panel"><PanelHeader title="正在运行" action="开始工作流" onAction={() => onCreate('workflowRuns', { title: `${titleFor(project)} · 工作链运行`, projectId: project.id, goalId: project.goalId, status: 'PLANNED', startedAt: nowInput() })} />{activeRuns.length ? activeRuns.slice(0, 5).map((run) => <CompactRecord key={run.id} record={run} onOpen={onOpen} />) : <p className="empty-copy">当前没有运行中的工作链。</p>}</section><section className="work-panel"><PanelHeader title="项目关注" action="查看执行" onAction={() => setTab('execution')} />{attentionLogs.length ? attentionLogs.slice(0, 3).map((log) => <CompactRecord key={log.id} record={log} onOpen={onOpen} />) : <p className="empty-copy">还没有异常或待验证假设。</p>}<PanelHeader title="最近调整" />{recentChanges.length ? recentChanges.slice(0, 3).map((log) => <CompactRecord key={log.id} record={log} onOpen={onOpen} />) : <p className="empty-copy">尚未记录行动或决策变化。</p>}</section></div></div>}
    {tab === 'execution' && <ProjectExecutionWorkbench project={project} records={records} tasks={tasks} logs={logs} onOpen={onOpen} onCreate={onCreate} onArchive={onArchive} onSave={onSave} />}
    {tab === 'data' && <ProjectDataWorkbench project={project} tables={tables} onOpen={onOpen} onCreate={onCreate} onSave={onSave} />}
    {tab === 'materials' && <ProjectMaterialsWorkbench project={project} records={records} materials={materials} onOpen={onOpen} onCreate={onCreate} onArchive={onArchive} onSave={onSave} />}
    {tab === 'review' && <ProjectReviewWorkbench project={project} records={records} related={related} onOpen={onOpen} onCreate={onCreate} />}
  </div>
}

function ProjectWorkbenchDialog({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  return <div className="overlay-backdrop project-workbench-dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="notebook-action-dialog project-workbench-dialog"><header><div><p>PROJECT WORKBENCH</p><h3>{title}</h3></div><button aria-label="关闭" onClick={onClose}>×</button></header>{children}</section></div>
}

function ProjectCapabilityPicker({ project, records, onApply, onClose }: { project: RecordData; records: RecordData[]; onApply: (pack: RecordData, projectId: string) => Promise<void>; onClose: () => void }) {
  const [query, setQuery] = useState(''); const [assetKind, setAssetKind] = useState('ALL'); const [selectedPackId, setSelectedPackId] = useState(''); const [applyingId, setApplyingId] = useState<string | null>(null)
  const appliedPackIds = new Set(records.filter((record) => record.entity === 'packApplications' && record.projectId === project.id && String(record.status) !== 'ARCHIVED').map((record) => String(record.packId || '')))
  const packs = records.filter((record) => record.entity === 'capabilityPacks' && String(record.status) !== 'ARCHIVED').filter((pack) => titleFor(pack).toLowerCase().includes(query.trim().toLowerCase())).filter((pack) => assetKind === 'ALL' || capabilityApplicationPlan(records, pack.id).some((item) => item.asset.assetKind === assetKind)).sort((a, b) => Number(appliedPackIds.has(b.id)) - Number(appliedPackIds.has(a.id)) || titleFor(a).localeCompare(titleFor(b), 'zh-CN'))
  const selected = packs.find((pack) => pack.id === selectedPackId) || records.find((record) => record.entity === 'capabilityPacks' && record.id === selectedPackId)
  const plan = selected ? capabilityApplicationPlan(records, selected.id) : []
  const preview = selected ? { tasks: plan.filter((item) => item.createsExecutionTask).length, checklists: plan.filter((item) => item.instanceKind === 'CHECKLIST').length, tables: plan.filter((item) => item.instanceKind === 'TABLE').length, documents: plan.filter((item) => item.instanceKind === 'DOCUMENT').length, unavailable: plan.filter((item) => !item.version).length } : null
  const apply = async () => { if (!selected || applyingId) return; setApplyingId(selected.id); try { await onApply(selected, project.id); onClose() } finally { setApplyingId(null) } }
  return <div className="project-capability-picker"><div className="capability-picker-filters"><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索能力包…" /><label>资产类型<select value={assetKind} onChange={(event) => { setAssetKind(event.target.value); setSelectedPackId('') }}><option value="ALL">全部</option><option value="CHECKLIST_TEMPLATE">清单</option><option value="TABLE_TEMPLATE">表格</option><option value="DOCUMENT_TEMPLATE">文档</option><option value="PLAYBOOK">Playbook</option><option value="SOP">SOP</option><option value="PATTERN">模式</option></select></label></div>{packs.length ? <div className="compact-record-list">{packs.map((pack) => <article key={pack.id} className={selected?.id === pack.id ? 'selected' : ''}><button className="capability-pack-choice" onClick={() => setSelectedPackId(pack.id)}><strong>{titleFor(pack)}</strong><small>{appliedPackIds.has(pack.id) ? '已应用过；可再次基于当前版本创建实例' : String(pack.description || '点击预览将要创建的项目专属实例。')}</small></button><button className="button" onClick={() => setSelectedPackId(pack.id)}>预览</button></article>)}</div> : <p className="empty-copy">没有可用能力包。请先在成果中心创建并发布能力资产。</p>}{selected && preview && <section className="capability-apply-preview"><div><strong>应用预览：{titleFor(selected)}</strong><small>将创建项目专属快照；不会修改能力模板或既有实例。</small></div><p>{plan.length} 项能力资产 · {preview.tasks} 个执行任务 · {preview.checklists} 个清单 · {preview.tables} 张表格 · {preview.documents} 份文档{preview.unavailable ? ` · ${preview.unavailable} 项版本不可用` : ''}</p><footer><button className="button ghost" onClick={() => setSelectedPackId('')}>取消</button><button className="button primary" disabled={Boolean(applyingId) || Boolean(preview.unavailable)} onClick={() => void apply()}>{applyingId ? '应用中…' : '确认应用'}</button></footer></section>}</div>
}

function ProjectOperatingState({ project, onSave }: { project: RecordData; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [editing, setEditing] = useState(false)
  const [draft, setDraft] = useState(() => projectStateDraft(project))
  useEffect(() => setDraft(projectStateDraft(project)), [project])
  const save = async () => { await onSave('projects', { ...project, ...draft }); setEditing(false) }
  const fields: Array<[keyof typeof draft, string, boolean?]> = [['stage', '当前阶段'], ['currentStrategy', '当前策略', true], ['currentFocus', '当前聚焦', true], ['currentProblem', '当前问题', true], ['nextAction', '下一步行动', true]]
  return <section className="work-panel operating-state"><PanelHeader title="项目当前经营状态" action={editing ? '取消' : '更新状态'} onAction={() => setEditing((value) => !value)} />{editing ? <div className="operating-state-form">{fields.map(([key, label, multiline]) => <label key={key}><span>{label}</span>{multiline ? <textarea value={draft[key]} onChange={(event) => setDraft((value) => ({ ...value, [key]: event.target.value }))} /> : <input value={draft[key]} onChange={(event) => setDraft((value) => ({ ...value, [key]: event.target.value }))} />}</label>)}<button className="button primary" onClick={() => void save()}>保存当前状态</button></div> : <div className="operating-state-grid">{fields.map(([key, label]) => <article key={key}><small>{label}</small><p>{draft[key] || '尚未记录'}</p></article>)}</div>}</section>
}

function ProjectExecutionWorkbench({ project, records, tasks, logs, onOpen, onCreate, onArchive, onSave }: { project: RecordData; records: RecordData[]; tasks: RecordData[]; logs: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [typeFilter, setTypeFilter] = useState('ALL'); const [dateFilter, setDateFilter] = useState('')
  const filteredLogs = logs.filter((log) => (typeFilter === 'ALL' || log.type === typeFilter) && (!dateFilter || localDateKey(String(log.happenedAt || log.updatedAt || '')) === dateFilter)).sort((a, b) => String(b.happenedAt || b.updatedAt).localeCompare(String(a.happenedAt || a.updatedAt)))
  const checklists = records.filter((record) => record.entity === 'templateInstances' && record.projectId === project.id && record.instanceKind === 'CHECKLIST')
  return <section className="project-execution"><OperationalQuickCapture project={project} records={records} onSave={onSave} /><div className="three-column"><section className="work-panel"><PanelHeader title="执行任务" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />{tasks.length ? tasks.filter(isActive).map((task) => <CompactRecord key={task.id} record={task} onOpen={onOpen} />) : <p className="empty-copy">这里直接引用项目 Task，不创建第二套任务系统。</p>}</section><section className="work-panel"><PanelHeader title="当前工作链" action="管理运行" onAction={() => onCreate('workflowRuns', { title: `${titleFor(project)} · 工作链运行`, projectId: project.id, goalId: project.goalId, status: 'PLANNED', startedAt: nowInput() })} /><ProjectRunSummary project={project} records={records} onOpen={onOpen} /></section><section className="work-panel"><PanelHeader title="执行清单" action="新建清单" onAction={() => onCreate('templateInstances', { title: `${titleFor(project)} · 执行清单`, instanceKind: 'CHECKLIST', projectId: project.id, status: 'DRAFT', itemsSnapshot: '[]' })} />{checklists.length ? checklists.map((item) => <CompactRecord key={item.id} record={item} onOpen={onOpen} />) : <p className="empty-copy">清单引用能力实例；不会复制项目任务。</p>}</section></div><section className="work-panel"><PanelHeader title="工作事件流" action="新建完整记录" onAction={() => onCreate('operationalLogs', { projectId: project.id, type: 'NOTE', happenedAt: nowInput() })} /><div className="operation-log-filters"><label>类型<select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="ALL">全部类型</option>{['NOTE', 'OBSERVATION', 'ACTION', 'ISSUE', 'HYPOTHESIS', 'DECISION', 'METRIC'].map((type) => <option key={type} value={type}>{operationalTypeLabel(type)}</option>)}</select></label><label>日期<input type="date" value={dateFilter} onChange={(event) => setDateFilter(event.target.value)} /></label><button className="button ghost" onClick={() => { setTypeFilter('ALL'); setDateFilter('') }}>重置</button></div>{filteredLogs.length ? filteredLogs.map((log) => <div className="operation-log-row" key={log.id}><button onClick={() => onOpen(log)}><em>{operationalTypeLabel(log.type)}</em><span>{String(log.content || '未命名记录')}</span><small>{formatDate(log.happenedAt || log.updatedAt, true)}</small></button><button className="icon-action" aria-label="归档执行记录" onClick={() => onArchive(log.id)}>×</button></div>) : <p className="empty-copy">没有匹配的执行记录。写下一句话即可保存；分类、关联任务和人物都不是必填项。</p>}</section><ProjectWorkChain project={project} records={records} onOpen={onOpen} onCreate={onCreate} onSave={onSave} /></section>
}

function OperationalQuickCapture({ project, records, onSave, onSaved, defaultWorkflowRunId }: { project: RecordData; records: RecordData[]; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void>; onSaved?: () => void; defaultWorkflowRunId?: string }) {
  const [content, setContent] = useState(''); const [saving, setSaving] = useState(false); const [more, setMore] = useState(false); const [type, setType] = useState('NOTE'); const [workflowRunId, setWorkflowRunId] = useState(defaultWorkflowRunId || ''); const [taskId, setTaskId] = useState('')
  useEffect(() => setWorkflowRunId(defaultWorkflowRunId || ''), [defaultWorkflowRunId])
  const save = async () => { const value = content.trim(); if (!value || saving) return; setSaving(true); try { await onSave('operationalLogs', { content: value, type, projectId: project.id, happenedAt: new Date().toISOString(), workflowRunId: workflowRunId || undefined, taskId: taskId || undefined }); setContent(''); setMore(false); setType('NOTE'); setWorkflowRunId(defaultWorkflowRunId || ''); setTaskId(''); onSaved?.() } finally { setSaving(false) } }
  const runs = records.filter((record) => record.entity === 'workflowRuns' && record.projectId === project.id); const tasks = records.filter((record) => record.entity === 'tasks' && record.projectId === project.id)
  return <section className="work-panel quick-operational-log"><header><div><h3>快速记录</h3><p>先捕捉发生了什么；默认保存为笔记。</p></div><button className="button ghost" onClick={() => setMore((value) => !value)}>{more ? '收起补充' : '补充关联'}</button></header><div className="quick-log-input"><textarea aria-label="快速记录内容" value={content} placeholder="记录执行中的观察、行动、问题或数据变化…" onChange={(event) => setContent(event.target.value)} /><button className="button primary" disabled={!content.trim() || saving} onClick={() => void save()}>{saving ? '保存中…' : '保存'}</button></div>{more && <div className="quick-log-more"><label>类型<select value={type} onChange={(event) => setType(event.target.value)}>{['NOTE', 'OBSERVATION', 'ACTION', 'ISSUE', 'HYPOTHESIS', 'DECISION', 'METRIC'].map((value) => <option key={value} value={value}>{operationalTypeLabel(value)}</option>)}</select></label>{defaultWorkflowRunId ? <p className="quick-log-run-lock">已自动关联当前工作链：{runs.find((item) => item.id === defaultWorkflowRunId) ? titleFor(runs.find((item) => item.id === defaultWorkflowRunId)!) : '当前运行'}</p> : <label>关联工作链<select value={workflowRunId} onChange={(event) => setWorkflowRunId(event.target.value)}><option value="">不关联</option>{runs.map((item) => <option key={item.id} value={item.id}>{titleFor(item)}</option>)}</select></label>}<label>关联任务<select value={taskId} onChange={(event) => setTaskId(event.target.value)}><option value="">不关联</option>{tasks.map((item) => <option key={item.id} value={item.id}>{titleFor(item)}</option>)}</select></label></div>}</section>
}

function ProjectRunSummary({ project, records, onOpen }: { project: RecordData; records: RecordData[]; onOpen: (record: RecordData) => void }) {
  const runs = records.filter((record) => record.entity === 'workflowRuns' && record.projectId === project.id); const active = runs.find((item) => ['RUNNING', 'PLANNED', 'PAUSED'].includes(String(item.status)))
  return active ? <CompactRecord record={active} onOpen={onOpen} /> : <p className="empty-copy">尚未应用工作链。需要时创建实际运行，模板不会被直接改写。</p>
}

function ProjectDataWorkbench({ project, tables, onOpen, onCreate, onSave }: { project: RecordData; tables: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [selectedId, setSelectedId] = useState<string | null>(tables[0]?.id || null)
  useEffect(() => { if (!tables.some((item) => item.id === selectedId)) setSelectedId(tables[0]?.id || null) }, [selectedId, tables])
  const selected = tables.find((item) => item.id === selectedId)
  return <section className="project-data"><header className="workbench-section-header"><div><p className="eyebrow">STRUCTURED TABLES</p><h3>项目数据</h3><p>复用能力包应用产生的表格实例；数据归属于本项目。</p></div><button className="button primary" onClick={() => onCreate('templateInstances', { title: `${titleFor(project)} · 数据表`, instanceKind: 'TABLE', projectId: project.id, status: 'DRAFT', schemaSnapshot: JSON.stringify(['内容']), rowsData: '[]' })}>＋ 新建表格</button></header>{tables.length ? <div className="project-table-layout"><aside>{tables.map((table) => <button key={table.id} className={table.id === selected?.id ? 'active' : ''} onClick={() => setSelectedId(table.id)}><strong>{titleFor(table)}</strong><small>{String(table.status || 'DRAFT')}</small></button>)}</aside>{selected && <ProjectTableEditor table={selected} onOpen={onOpen} onSave={onSave} />}</div> : <GuidedEmpty icon="▤" title="还没有项目表格" text="可新建一个项目表格，或在能力包中应用表格模板。数据不会回写能力模板。" action="新建表格" onAction={() => onCreate('templateInstances', { title: `${titleFor(project)} · 数据表`, instanceKind: 'TABLE', projectId: project.id, status: 'DRAFT', schemaSnapshot: JSON.stringify(['内容']), rowsData: '[]' })} />}</section>
}

function ProjectTableEditor({ table, onOpen, onSave }: { table: RecordData; onOpen: (record: RecordData) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [rows, setRows] = useState<Record<string, string>[]>(() => tableRows(table)); const [saving, setSaving] = useState(false)
  useEffect(() => setRows(tableRows(table)), [table])
  const columns = tableColumns(table, rows)
  const update = (index: number, key: string, value: string) => setRows((items) => items.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: value } : row))
  const save = async () => { setSaving(true); try { await onSave('templateInstances', { ...table, rowsData: JSON.stringify(rows) }) } finally { setSaving(false) } }
  return <article className="project-table-editor"><header><div><h3>{titleFor(table)}</h3><p>每次保存只更新本项目的表格实例。</p></div><button className="button ghost" onClick={() => onOpen(table)}>完整编辑</button></header><div className="project-table-scroll"><table><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}<th /></tr></thead><tbody>{rows.map((row, index) => <tr key={index}>{columns.map((column) => <td key={column}><input value={row[column] || ''} onChange={(event) => update(index, column, event.target.value)} /></td>)}<td><button className="icon-action" aria-label="删除行" onClick={() => setRows((items) => items.filter((_, rowIndex) => rowIndex !== index))}>×</button></td></tr>)}</tbody></table></div><footer><button className="button ghost" onClick={() => setRows((items) => [...items, Object.fromEntries(columns.map((column) => [column, '']))])}>＋ 新增行</button><button className="button primary" onClick={() => void save()} disabled={saving}>{saving ? '保存中…' : '保存数据'}</button></footer></article>
}

function ProjectMaterialsWorkbench({ project, records, materials, onOpen, onCreate, onArchive, onSave }: { project: RecordData; records: RecordData[]; materials: Array<{ deliverable: RecordData; file?: RecordData }>; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [fileId, setFileId] = useState(''); const [folderId, setFolderId] = useState(''); const [folderName, setFolderName] = useState(''); const [scope, setScope] = useState('PROJECT'); const [busy, setBusy] = useState(false); const fileInputRef = useRef<HTMLInputElement>(null)
  const files = records.filter((record) => record.entity === 'notebookFiles' && !record.archivedAt && !record.deletedAt); const folders = records.filter((record) => record.entity === 'notebookFolders' && record.projectId === project.id && !record.archivedAt && !record.deletedAt).sort((a, b) => titleFor(a).localeCompare(titleFor(b), 'zh-CN')); const selectedFolder = folders.find((folder) => folder.id === folderId)
  useEffect(() => { if (folderId && !folders.some((folder) => folder.id === folderId)) setFolderId('') }, [folderId, folders])
  const link = async () => { const file = files.find((item) => item.id === fileId); if (!file || busy) return; setBusy(true); try { await onSave('deliverables', { title: titleFor(file), fileId: file.id, notebookFolderId: folderId || undefined, projectId: project.id, goalId: project.goalId, status: 'DRAFT', assetType: 'OTHER', contextScope: scope, versionNumber: 'v1', summary: '项目资料引用；原始文件仍由收纳箱管理。' }); setFileId('') } finally { setBusy(false) } }
  const createFolder = async () => { const name = folderName.trim(); if (!name || busy) return; setBusy(true); try { await onSave('notebookFolders', { name, projectId: project.id, sortOrder: Date.now() }); setFolderName('') } finally { setBusy(false) } }
  const renameFolder = async () => { const name = folderName.trim(); if (!selectedFolder || !name || busy) return; setBusy(true); try { await onSave('notebookFolders', { ...selectedFolder, name }); setFolderName('') } finally { setBusy(false) } }
  const importFiles = async (items: FileList | null) => { if (!items?.length || busy) return; setBusy(true); try { for (const file of Array.from(items)) { const stored = await api.uploadNotebookFile({ file }); await onSave('deliverables', { title: titleFor(stored), fileId: stored.id, notebookFolderId: folderId || undefined, projectId: project.id, goalId: project.goalId, status: 'DRAFT', assetType: 'OTHER', contextScope: scope, versionNumber: 'v1', summary: '从项目资料导入；原始文件由收纳箱 FileAsset 管理。' }) } } finally { if (fileInputRef.current) fileInputRef.current.value = ''; setBusy(false) } }
  const visible = materials.filter(({ deliverable }) => !folderId || deliverable.notebookFolderId === folderId)
  return <section className="project-materials"><input ref={fileInputRef} className="visually-hidden" type="file" multiple onChange={(event) => void importFiles(event.target.files)} /><header className="workbench-section-header"><div><p className="eyebrow">MATERIALS · FILE REFERENCES</p><h3>项目资料</h3><p>资料只引用收纳箱 FileAsset，不复制文件；项目文件夹只组织引用，不移动原始文件。</p></div><div><button className="button" disabled={busy} onClick={() => fileInputRef.current?.click()}>导入文件</button><button className="button primary" onClick={() => onCreate('deliverables', { projectId: project.id, goalId: project.goalId, status: 'DRAFT', assetType: 'OTHER', contextScope: scope, versionNumber: 'v1' })}>＋ 添加资料</button></div></header><div className="material-folder-toolbar"><select aria-label="项目资料文件夹" value={folderId} onChange={(event) => { setFolderId(event.target.value); setFolderName('') }}><option value="">全部资料</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{titleFor(folder)}</option>)}</select><input value={folderName} onChange={(event) => setFolderName(event.target.value)} placeholder={selectedFolder ? `重命名「${titleFor(selectedFolder)}」` : '新建项目资料文件夹'} /><button className="button" disabled={!folderName.trim() || busy} onClick={() => void (selectedFolder ? renameFolder() : createFolder())}>{selectedFolder ? '重命名' : '新建文件夹'}</button><label>资料归属<select value={scope} onChange={(event) => setScope(event.target.value)}><option value="PROJECT">项目</option><option value="COMPANY">公司</option><option value="PERSONAL">个人可复用</option></select></label></div><div className="material-linker"><select value={fileId} onChange={(event) => setFileId(event.target.value)}><option value="">选择收纳箱中的文件以关联</option>{files.map((file) => <option key={file.id} value={file.id}>{titleFor(file)}</option>)}</select><button className="button" disabled={!fileId || busy} onClick={() => void link()}>{busy ? '处理中…' : '关联已有文件'}</button></div>{visible.length ? <div className="material-list">{visible.map(({ deliverable, file }) => <article key={deliverable.id}><button onClick={() => onOpen(file || deliverable)}><span>⊞</span><div><strong>{titleFor(file || deliverable)}</strong><small>{file ? `${file.extension || '文件'} · ${file.size || 0} bytes · ${deliverable.contextScope === 'COMPANY' ? '公司' : deliverable.contextScope === 'PERSONAL' ? '个人可复用' : '项目'}` : '原始文件已不可用；请重新关联'}</small></div></button><select aria-label={`移动 ${titleFor(file || deliverable)}`} value={String(deliverable.notebookFolderId || '')} onChange={(event) => void onSave('deliverables', { ...deliverable, notebookFolderId: event.target.value || undefined })}><option value="">未分组</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{titleFor(folder)}</option>)}</select><button className="button ghost" onClick={() => onArchive(deliverable.id)}>取消关联</button></article>)}</div> : <GuidedEmpty icon="⊞" title={folderId ? '该文件夹暂无资料' : '尚未关联资料'} text="可从收纳箱选取已有文件，或导入新文件。取消关联不会删除原始文件。" />}</section>
}

function ProjectReviewWorkbench({ project, records, related, onOpen, onCreate }: { project: RecordData; records: RecordData[]; related: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void }) {
  const reviews = related.filter((record) => record.entity === 'reviews'); const insights = related.filter((record) => record.entity === 'insights'); const proposals = records.filter((record) => record.entity === 'capabilityImprovementProposals' && (record.projectId === project.id || related.some((item) => item.id === record.sourceReviewId || item.id === record.sourceInsightId || item.id === record.sourceResultId)))
  return <section className="project-review"><header className="workbench-section-header"><div><p className="eyebrow">REVIEW → INSIGHT → IMPROVEMENT</p><h3>项目复盘</h3><p>从真实结果和执行记录中学习；不会自动改写能力包或发布新版本。</p></div><div><button className="button" onClick={() => onCreate('reviews', { projectId: project.id, goalId: project.goalId })}>＋ 新建复盘</button><button className="button primary" onClick={() => onCreate('results', { projectId: project.id, goalId: project.goalId, status: 'PLANNED', date: today(), evidenceStatus: 'RECORDED' })}>＋ 记录结果</button></div></header><ProjectResults project={project} records={records} onOpen={onOpen} onCreate={onCreate} /><div className="three-column"><section className="work-panel"><PanelHeader title="复盘" action="新建复盘" onAction={() => onCreate('reviews', { projectId: project.id, goalId: project.goalId })} />{reviews.length ? reviews.map((record) => <CompactRecord key={record.id} record={record} onOpen={onOpen} />) : <p className="empty-copy">没有复盘时不生成虚假的学习结论。</p>}</section><section className="work-panel"><PanelHeader title="洞见" action="记录洞见" onAction={() => onCreate('insights', { projectId: project.id, goalId: project.goalId })} />{insights.length ? insights.map((record) => <CompactRecord key={record.id} record={record} onOpen={onOpen} />) : <p className="empty-copy">洞见必须来自真实复盘、结果或用户记录。</p>}</section><section className="work-panel"><PanelHeader title="能力改进提案" action="创建提案" onAction={() => onCreate('capabilityImprovementProposals', { title: `${titleFor(project)} · 能力改进`, sourceType: 'USER', status: 'DRAFT' })} />{proposals.length ? proposals.map((record) => <CompactRecord key={record.id} record={record} onOpen={onOpen} />) : <p className="empty-copy">提案只作为下一版草稿，不会自动覆盖长期能力资产。</p>}</section></div></section>
}

function projectStateDraft(project: RecordData) { return { stage: String(project.stage || ''), currentStrategy: String(project.currentStrategy || ''), currentFocus: String(project.currentFocus || ''), currentProblem: String(project.currentProblem || ''), nextAction: String(project.nextAction || '') } }
function operationalTypeLabel(value: unknown) { return ({ NOTE: '笔记', OBSERVATION: '观察', ACTION: '行动', ISSUE: '问题', HYPOTHESIS: '假设', DECISION: '决策', METRIC: '指标' } as Record<string, string>)[String(value || 'NOTE')] || '笔记' }
function tableRows(table: RecordData) { try { const value = JSON.parse(String(table.rowsData || '[]')); return Array.isArray(value) ? value.filter((row): row is Record<string, unknown> => Boolean(row) && typeof row === 'object').map((row) => Object.fromEntries(Object.entries(row).map(([key, value]) => [key, String(value ?? '')]))) : [] } catch { return [] } }
function tableColumns(table: RecordData, rows: Record<string, string>[]) { try { const value = JSON.parse(String(table.schemaSnapshot || '[]')); const columns = Array.isArray(value) ? value.map((item) => typeof item === 'string' ? item : String((item as { name?: unknown; title?: unknown }).name || (item as { title?: unknown }).title || '')).filter(Boolean) : []; if (columns.length) return columns } catch { /* Existing malformed snapshots remain readable through row keys. */ } return [...new Set(rows.flatMap((row) => Object.keys(row)))].slice(0, 12).length ? [...new Set(rows.flatMap((row) => Object.keys(row)))].slice(0, 12) : ['内容'] }
function projectMaterials(records: RecordData[], projectId: string): Array<{ deliverable: RecordData; file?: RecordData }> { const files = new Map<string, RecordData>(records.filter((record) => record.entity === 'notebookFiles').map((record): [string, RecordData] => [record.id, record])); return records.filter((record) => record.entity === 'deliverables' && record.projectId === projectId && record.fileId).map((deliverable): { deliverable: RecordData; file?: RecordData } => ({ deliverable, file: files.get(String(deliverable.fileId || '')) })) }
...
function ProjectWorkChain({ project, records, onOpen, onCreate, onSave }: { project: RecordData; records: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  ...
  const runLogs = records.filter((record) => record.entity === 'operationalLogs' && record.workflowRunId === activeRun?.id).sort((a, b) => String(b.happenedAt || b.updatedAt).localeCompare(String(a.happenedAt || a.updatedAt)))
...
function FinanceView({ records, onOpen, onCreate, onRefresh, onAskAi }: { records: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onRefresh: () => void; onAskAi: () => void }) {
  ...
  return <div className="finance-page">
    <section className="finance-hero"><div><span>{PRODUCT_NAME} · RESOURCE LAYER</span><h2>Financial Intelligence</h2><p>记录资金事实，并把 Time + Money + Outcome 转化为 CEO 决策证据。现金净流动与经营贡献不等于会计利润。</p></div><button className="button primary" onClick={createTransaction}>＋ 记录流水</button></section>
...
function NotebookView({ records, externalItems, captureConfig, onOpen, onRefresh, onNotice, onAi }: { records: RecordData[]; externalItems: ExternalItem[]; captureConfig: CaptureProviderConfig | null; onOpen: (record: RecordData) => void; onRefresh: () => Promise<void>; onNotice: (text: string, tone?: Notice['tone']) => void; onAi: (question: string, context: Partial<AgentContext>) => void }) {
  ...
  const [capabilityFolderId, setCapabilityFolderId] = useState('')
  ...
  const moveToCategory = async (categoryId: string) => {
    ...
    try {
      ...
      setMovePickerOpen(false); setSelectedIds([]); setScope(categoryId || 'inbox'); setFolderId(''); await onRefresh(); onNotice(`已将 ${categoryMoveSources.length} 项归类；不会自动关联 ${PRODUCT_NAME} 记录。`)
  ...
  const assignCategory = async (record: RecordData, categoryId: string) => {
    try {
      ...
      await onRefresh()
      onNotice(categoryId ? `已放入收纳箱分类；不会自动关联 ${PRODUCT_NAME} 记录。` : '已移出收纳箱分类。')
  ...
  const addSelectedToCapability = async () => {
    if (!selected || !capabilityPackId) return
    const duplicate = records.some((record) => record.entity === 'capabilityWorkspaceEntries' && record.packId === capabilityPackId && record.targetId === selected.id && !record.archivedAt && !record.deletedAt)
    if (duplicate) { onNotice('这条收纳内容已在所选能力包中。'); setCapabilityPickerOpen(false); return }
    try {
      await api.save('capabilityWorkspaceEntries', { packId: capabilityPackId, folderId: capabilityFolderId || undefined, targetType: selected.entity, targetId: selected.id, orderIndex: Date.now() })
      await api.addRelation(selected.id, capabilityPackId, 'capability:IN_WORKSPACE')
      setCapabilityPickerOpen(false); setCapabilityPackId(''); setCapabilityFolderId(''); await onRefresh()
      onNotice('已加入能力包；原收纳内容保持不变。')
    } catch (error) { onNotice(`加入能力包失败：${String(error)}`, 'danger') }
  }
  ...
  const askAiAboutFile = (mode: 'summary' | 'relations') => { if (!selected || selected.entity !== 'notebookFiles') return; onAi(mode === 'summary' ? `请只基于这份 Notebook 文件的已提取内容，给出结构化摘要、关键事实、待验证点与可沉淀的知识。不要修改任何内容。` : `请只基于这份 Notebook 文件的已提取内容和 ${PRODUCT_NAME} 现有记录，列出最多 5 个潜在关联及理由。不要创建关系；由用户决定是否关联。`, { currentRoute: 'notebook', currentEntityType: 'notebookFiles', currentEntityId: selected.id, selectedItems: [selected.id] }) }
  ...
  return <div className="notebook-page notebook-space">
      ...
      <aside className={`notebook-preview ${selected?.entity === 'notes' ? 'note-preview' : 'detail-preview'}${editorFullscreen && selected?.entity === 'notes' ? ' fullscreen' : ''}`}>
        {selected?.entity === 'notes' ? <><div className="notebook-capability-action"><button className="button" onClick={() => { setCapabilityPackId(''); setCapabilityFolderId(''); setCapabilityPickerOpen(true) }}>加入能力包</button></div><RichNotebookEditor note={selected} records={records} related={related} categories={categories} fullscreen={editorFullscreen} onOpen={onOpen} onCreateNote={() => void createManualNote()} onRefresh={onRefresh} onNotice={onNotice} onAi={onAi} onAssignCategory={assignCategory} onCreateCategory={() => { setCategoryDraft(''); setCategoryDialogOpen(true) }} onToggleFullscreen={() => setEditorFullscreen((current) => !current)} onClose={() => { setEditorFullscreen(false); setSelectedId('__closed__') }} onRelationsChanged={() => void api.relations(selected.id).then(setRelated)} onRegisterFlush={(value) => { editorFlushRef.current = value }} /></> : <>
          ...
          {selected ? <>
            ...
            {['notes', 'notebookFiles', 'inbox'].includes(selected.entity) && <button className="button" onClick={() => { setCapabilityPackId(''); setCapabilityFolderId(''); setCapabilityPickerOpen(true) }}>加入能力包</button>}
            ...
            {selected.entity === 'notebookFiles' && <div className="notebook-file-actions"><button className="button" onClick={openSelectedFile}>用原应用打开</button><button className="button" onClick={revealSelectedFile}>在 Finder 显示</button><button className="button" onClick={() => void copySelected()}>复制</button><button className="button" onClick={() => void refreshExtractedContent()}>重新提取</button><button className="button" onClick={() => askAiAboutFile('summary')}>AI 理解</button></div>}
            <section className="notebook-relation-box"><h4>关联 {PRODUCT_NAME} <span>可选</span></h4><p>只有你确认后才会建立关联。</p><select value={relationTarget} onChange={(event) => setRelationTarget(event.target.value)}><option value="">选择要关联的对象</option>{relationTargets.map((target) => <option key={target.id} value={target.id}>{configFor(target.entity).label} · {titleFor(target)}</option>)}</select><button className="button primary" disabled={!relationTarget} onClick={() => void addOptionalRelation()}>确认关联</button></section>
    ...
    {capabilityPickerOpen && selected && <div className="overlay-backdrop notebook-dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setCapabilityPickerOpen(false)}><section className="notebook-action-dialog"><header><div><p>能力资产</p><h3>加入能力包</h3></div><button onClick={() => setCapabilityPickerOpen(false)}>×</button></header><label><span>能力包</span><select value={capabilityPackId} onChange={(event) => { setCapabilityPackId(event.target.value); setCapabilityFolderId('') }}><option value="">选择能力包</option>{records.filter((record) => record.entity === 'capabilityPacks' && !record.archivedAt && !record.deletedAt).map((pack) => <option key={pack.id} value={pack.id}>{titleFor(pack)}</option>)}</select></label>{capabilityPackId && <label><span>工作区文件夹</span><select value={capabilityFolderId} onChange={(event) => setCapabilityFolderId(event.target.value)}><option value="">根目录</option>{records.filter((record) => record.entity === 'capabilityWorkspaceFolders' && record.packId === capabilityPackId && !record.archivedAt && !record.deletedAt).map((folder) => <option key={folder.id} value={folder.id}>{titleFor(folder)}</option>)}</select></label>}<small>只创建工作区引用，原收纳内容不会移动、复制或删除。</small><footer><button className="button ghost" onClick={() => setCapabilityPickerOpen(false)}>取消</button><button className="button primary" disabled={!capabilityPackId} onClick={() => void addSelectedToCapability()}>确认加入</button></footer></section></div>}
  </div>
}
...
function ResearchInboxView({ records, externalItems: _externalItems, categories, captureConfig, onBackToCapture, onRefresh, onNotice }: { records: RecordData[]; externalItems: ExternalItem[]; categories: RecordData[]; captureConfig: CaptureProviderConfig | null; onBackToCapture: () => void; onRefresh: () => Promise<void>; onNotice: (text: string, tone?: Notice['tone']) => void }) {
  ...
  const createPlan = async (draft = requestDraft, parentThreadId = '') => {
    ...
    try {
      ...
      onNotice(parentThreadId ? '已在同一调研主题下生成后续方案；历史结果不会被覆盖。' : `已生成调研方案；确认前不会抓取数据或关联 ${PRODUCT_NAME}。`)
  ...
  return <div className="notebook-page notebook-space research-inbox-page">
    ...
    <section className="research-request-composer">
      ...
      <div><small>系统会先规划情报源；确认前不会抓取数据或关联 {PRODUCT_NAME}。</small><button className="button primary" disabled={!requestDraft.trim() || busy} onClick={() => void createPlan()}>{busy ? '正在规划…' : '生成调研方案'}</button></div>
    </section>
