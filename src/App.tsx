...
function ProjectCapabilityPicker({ project, records, onApply, onClose }: { project: RecordData; records: RecordData[]; onApply: (pack: RecordData, projectId: string) => Promise<void>; onClose: () => void }) {
  const [query, setQuery] = useState(''); const [applyingId, setApplyingId] = useState<string | null>(null)
  ...
  const packs = records.filter((record) => record.entity === 'capabilityPacks' && String(record.status) !== 'ARCHIVED').filter((pack) => titleFor(pack).toLowerCase().includes(query.trim().toLowerCase())).sort((a, b) => Number(appliedPackIds.has(b.id)) - Number(appliedPackIds.has(a.id)) || titleFor(a).localeCompare(titleFor(b), 'zh-CN'))
  const apply = async (pack: RecordData) => { if (applyingId) return; setApplyingId(pack.id); try { await onApply(pack, project.id); onClose() } finally { setApplyingId(null) } }
  return <div className="project-capability-picker"><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索能力包…" />{packs.length ? <div className="compact-record-list">{packs.map((pack) => <article key={pack.id}><div><strong>{titleFor(pack)}</strong><small>{appliedPackIds.has(pack.id) ? '已应用过；可再次基于当前版本创建实例' : String(pack.description || '能力包会创建项目专属实例，不会修改能力模板。')}</small></div><button className="button primary" disabled={Boolean(applyingId)} onClick={() => void apply(pack)}>{applyingId === pack.id ? '应用中…' : '应用'}</button></article>)}</div> : <p className="empty-copy">没有可用能力包。请先在成果中心创建并发布能力资产。</p>}</div>
}
...
function ProjectMaterialsWorkbench({ project, records, materials, onOpen, onCreate, onArchive, onSave }: { project: RecordData; records: RecordData[]; materials: Array<{ deliverable: RecordData; file?: RecordData }>; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const [fileId, setFileId] = useState(''); const [linking, setLinking] = useState(false); const files = records.filter((record) => record.entity === 'notebookFiles')
  const link = async () => { const file = files.find((item) => item.id === fileId); if (!file || linking) return; setLinking(true); try { await onSave('deliverables', { title: titleFor(file), fileId: file.id, projectId: project.id, goalId: project.goalId, status: 'DRAFT', assetType: 'OTHER', versionNumber: 'v1', summary: '项目资料引用；原始文件仍由收纳箱管理。' }); setFileId('') } finally { setLinking(false) } }
  return <section className="project-materials"><header className="workbench-section-header"><div><p className="eyebrow">MATERIALS · FILE REFERENCES</p><h3>项目资料</h3><p>资料只引用收纳箱 FileAsset，不复制文件，也不改变原始文件。</p></div><div><button className="button" onClick={() => onCreate('notebookFiles', {})}>导入文件</button><button className="button primary" onClick={() => onCreate('deliverables', { projectId: project.id, goalId: project.goalId, status: 'DRAFT', assetType: 'OTHER', versionNumber: 'v1' })}>＋ 添加资料</button></div></header><div className="material-linker"><select value={fileId} onChange={(event) => setFileId(event.target.value)}><option value="">选择收纳箱中的文件以关联</option>{files.map((file) => <option key={file.id} value={file.id}>{titleFor(file)}</option>)}</select><button className="button" disabled={!fileId || linking} onClick={() => void link()}>{linking ? '关联中…' : '关联已有文件'}</button></div>{materials.length ? <div className="material-list">{materials.map(({ deliverable, file }) => <article key={deliverable.id}><button onClick={() => onOpen(file || deliverable)}><span>⊞</span><div><strong>{titleFor(file || deliverable)}</strong><small>{file ? `${file.extension || '文件'} · ${file.size || 0} bytes` : '原始文件已不可用；请重新关联'}</small></div></button><button className="button ghost" onClick={() => onArchive(deliverable.id)}>取消关联</button></article>)}</div> : <GuidedEmpty icon="⊞" title="尚未关联资料" text="可从收纳箱选取已有文件，或导入新文件。取消关联不会删除原始文件。" />}</section>
}
