...
function App() {
  ...
  return <div className={`app-shell ${view === 'today' ? 'today-route' : ''} ${view === 'finance' ? 'finance-route' : ''} ${view === 'time' ? 'time-route' : ''} ${activeTheme === 'light' ? 'light-theme' : 'dark-theme'} ${effectiveSidebarOpen ? "" : "sidebar-collapsed"} ${sidebarPeek ? "sidebar-peek" : ""} ${aiOpen ? "ai-open" : ""} ${aiResizing ? "ai-resizing" : ""}`} style={{ "--ai-width": `${aiWidth}px` } as React.CSSProperties} onPointerMove={handleShellPointer}>
    ...
    <main className="main-content">
      ...
      {view === 'projects' && <ProjectsView records={records} selectedId={selectedProjectId} onSelect={setSelectedProjectId} onOpen={openRecord} onCreate={openCreate} onEdit={(record) => setEditing({ config: configFor(record.entity), record })} onStartTimer={startTimer} onArchive={archiveRecord} onSave={async (entity, data) => { await api.save(entity, data); await refresh(); showNotice('已保存到本机。') }} />}
...
function ProjectsView({ records, selectedId, onSelect, onOpen, onCreate, onEdit, onStartTimer, onArchive, onSave }: { records: RecordData[]; selectedId: string | null; onSelect: (id: string | null) => void; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onEdit: (record: RecordData) => void; onStartTimer: (record: RecordData) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  const projects = records.filter((record) => record.entity === 'projects')
  const selected = projects.find((project) => project.id === selectedId)
  if (selected) return <ProjectWorkspace project={selected} records={records} onBack={() => onSelect(null)} onOpen={onOpen} onCreate={onCreate} onEdit={onEdit} onStartTimer={onStartTimer} onArchive={onArchive} onSave={onSave} />
...
function ProjectWorkspace({ project, records, onBack, onOpen, onCreate, onEdit, onStartTimer, onArchive, onSave }: { project: RecordData; records: RecordData[]; onBack: () => void; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onEdit: (record: RecordData) => void; onStartTimer: (record: RecordData) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  ...
  return <div className="project-workspace project-workbench"><button className="back-link" onClick={onBack}>← 返回项目</button><section className="workspace-hero"><div><div className="meta-line"><span className={`health ${project.health || 'healthy'}`} />{statusLabel(project.status)} · {relationName(project.goalId, records) || '未关联目标'}</div><h2>{titleFor(project)}</h2><p>{String(project.why || project.description || '尚未填写项目为什么值得投入。')}</p></div><div><button className="button ghost" onClick={() => onEdit(project)}>编辑项目</button><button className="button primary" onClick={() => onStartTimer(project)}>▶ 为项目计时</button></div></section><nav className="workspace-tabs">{tabs.map((item) => <button key={item.key} className={tab === item.key ? 'active' : ''} onClick={() => setTab(item.key)}>{item.label}{item.count !== undefined && <span>{item.count}</span>}</button>)}</nav>
    {tab === 'overview' && <div className="workspace-overview project-workbench-overview"><div className="metric-strip"><Metric label="项目进度" value={`${percent(project.progress)}%`} /><Metric label="实际投入" value={formatMinutes(time)} /><Metric label="当前运行" value={runs.filter((item) => ['RUNNING', 'PLANNED', 'PAUSED'].includes(String(item.status))).length ? '进行中' : '未开始'} /><Metric label="真实结果" value={results.length ? `${results.length} 条` : '未记录'} hint={economics.postedTransactions ? formatMoneyMinor(economics.managementContributionMinor) : '不以估算替代事实'} /></div><ProjectOperatingState project={project} onSave={onSave} /><div className="two-column"><section className="work-panel"><PanelHeader title="今天可推进的任务" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />{tasks.filter(isActive).length ? tasks.filter(isActive).slice(0, 5).map((task) => <CompactRecord key={task.id} record={task} onOpen={onOpen} />) : <GuidedEmpty icon="□" title="还没有执行任务" text="任务是下一步行动；不创建第二套任务系统。" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />}</section><section className="work-panel"><PanelHeader title="最近执行记录" action="查看执行" onAction={() => setTab('execution')} />{logs.length ? logs.sort((a, b) => String(b.happenedAt || b.updatedAt).localeCompare(String(a.happenedAt || a.updatedAt))).slice(0, 5).map((log) => <CompactRecord key={log.id} record={log} onOpen={onOpen} />) : <p className="empty-copy">先快速记录发生了什么；类型和关联可稍后再补。</p>}</section></div></div>}
...
function ProjectExecutionWorkbench({ project, records, tasks, logs, onOpen, onCreate, onArchive, onSave }: { project: RecordData; records: RecordData[]; tasks: RecordData[]; logs: RecordData[]; onOpen: (record: RecordData) => void; onCreate: (entity: Entity, initial?: Partial<RecordData>) => void; onArchive: (id: string) => void; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  return <section className="project-execution"><OperationalQuickCapture project={project} records={records} onSave={onSave} /><div className="three-column"><section className="work-panel"><PanelHeader title="执行任务" action="添加任务" onAction={() => onCreate('tasks', { projectId: project.id, goalId: project.goalId, status: 'todo' })} />{tasks.length ? tasks.filter(isActive).map((task) => <CompactRecord key={task.id} record={task} onOpen={onOpen} />) : <p className="empty-copy">这里直接引用项目 Task，不创建第二套任务系统。</p>}</section><section className="work-panel"><PanelHeader title="执行记录" action="新建完整记录" onAction={() => onCreate('operationalLogs', { projectId: project.id, type: 'NOTE', happenedAt: nowInput() })} />{logs.length ? logs.sort((a, b) => String(b.happenedAt || b.updatedAt).localeCompare(String(a.happenedAt || a.updatedAt))).map((log) => <div className="operation-log-row" key={log.id}><button onClick={() => onOpen(log)}><em>{operationalTypeLabel(log.type)}</em><span>{String(log.content || '未命名记录')}</span><small>{formatDate(log.happenedAt || log.updatedAt, true)}</small></button><button className="icon-action" aria-label="归档执行记录" onClick={() => onArchive(log.id)}>×</button></div>) : <p className="empty-copy">写下一句话即可保存。分类、关联任务和人物都不是必填项。</p>}</section><section className="work-panel"><PanelHeader title="当前工作链" action="管理运行" onAction={() => onCreate('workflowRuns', { title: `${titleFor(project)} · 工作链运行`, projectId: project.id, goalId: project.goalId, status: 'PLANNED', startedAt: nowInput() })} /><ProjectRunSummary project={project} records={records} onOpen={onOpen} /></section></div><ProjectWorkChain project={project} records={records} onOpen={onOpen} onCreate={onCreate} /></section>
}

function OperationalQuickCapture({ project, records, onSave }: { project: RecordData; records: RecordData[]; onSave: (entity: Entity, data: Partial<RecordData>) => Promise<void> }) {
  ...
  const save = async () => { const value = content.trim(); if (!value || saving) return; setSaving(true); try { await onSave('operationalLogs', { content: value, type, projectId: project.id, happenedAt: new Date().toISOString(), workflowRunId: workflowRunId || undefined, taskId: taskId || undefined }); setContent(''); setMore(false); setType('NOTE'); setWorkflowRunId(''); setTaskId('') } finally { setSaving(false) } }
