...
export default function SettingsWorkspace(props: Props) {
  ...
  return <div className="settings-workspace"><header className="settings-workspace-head"><div><p>{PRODUCT_NAME} · SETTINGS</p><h1>设置与工作区</h1><small>管理身份、偏好、AI、同步与本地数据。</small></div></header><div className="settings-workspace-body"><nav aria-label="设置分类">{Array.from(new Set(sections.map((item) => item.group))).map((group) => <section key={group}><p>{group}</p>{sections.filter((item) => item.group === group).map((item) => <button key={item.id} className={section === item.id ? 'active' : ''} onClick={() => select(item.id)}>{item.label}</button>)}</section>)}</nav><main>{renderBody()}</main></div></div>
}
