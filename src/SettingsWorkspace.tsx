...
const sections: Array<{ id: SettingsSection; label: string; group: string }> = [
  { id: 'account', label: '账户', group: '账户' }, { id: 'general', label: '通用', group: '账户' }, { id: 'appearance', label: '外观', group: '偏好' }, { id: 'ai', label: 'AI 与模型', group: '智能' }, { id: 'voice', label: '语音', group: '智能' }, { id: 'sync', label: '同步与设备', group: '数据' }, { id: 'data', label: '数据与备份', group: '数据' }, { id: 'storage', label: '存储', group: '数据' }, { id: 'notifications', label: '通知', group: '偏好' }, { id: 'providers', label: 'Provider / API', group: '智能' }, { id: 'privacy', label: '隐私与安全', group: '安全' }, { id: 'shortcuts', label: '快捷键', group: '偏好' }, { id: 'help', label: '帮助与反馈', group: '支持' }, { id: 'about', label: `关于 ${PRODUCT_NAME}`, group: '支持' }, { id: 'logout', label: '退出登录', group: '账户' },
]
...
export default function SettingsWorkspace(props: Props) {
  ...
  const [profileDraft, setProfileDraft] = useState(() => ({ nickname: String(props.profile?.nickname || ''), displayName: String(props.profile?.displayName || ''), name: String(props.profile?.name || ''), email: String(props.profile?.email || ''), avatar: String(props.profile?.avatar || '') }))
  ...
  const saveAccount = async () => { setSavingProfile(true); try { await props.onSaveProfile(profileDraft); props.onNotice('账户资料已保存到本机。') } finally { setSavingProfile(false) } }
  const renderBody = () => {
    ...
    if (section === 'about') return <section className="settings-card"><h2>关于 {PRODUCT_NAME}</h2><p>本机优先的个人操作系统。以下为当前运行版本的真实 provenance。</p>{provenance ? <dl className="provenance-list"><div><dt>应用路径</dt><dd>{provenance.appPath}</dd></div><div><dt>版本</dt><dd>{provenance.appVersion}</dd></div><div><dt>Git Commit</dt><dd>{provenance.gitCommit}</dd></div><div><dt>Schema / Protocol</dt><dd>{provenance.schemaVersion} / {provenance.syncProtocolVersion}</dd></div></dl> : <p>正在读取版本信息…</p>}</section>
  ...
  return <div className="settings-workspace"><header className="settings-workspace-head"><div><p>EVOPOLIT · SETTINGS</p><h1>设置与工作区</h1><small>管理身份、偏好、AI、同步与本地数据。</small></div></header><div className="settings-workspace-body"><nav aria-label="设置分类">{Array.from(new Set(sections.map((item) => item.group))).map((group) => <section key={group}><p>{group}</p>{sections.filter((item) => item.group === group).map((item) => <button key={item.id} className={section === item.id ? 'active' : ''} onClick={() => select(item.id)}>{item.label}</button>)}</section>)}</nav><main>{renderBody()}</main></div></div>
}
