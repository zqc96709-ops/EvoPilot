...
export const entities: EntityConfig[] = [
  { entity: 'profiles', label: '我的档案', singular: '档案', icon: '◌', titleKey: 'title', description: 'EvoPilot 的长期个人与 AI 上下文；不作为业务模块或左侧导航。', fields: [
  ...
  { entity: 'results', label: '成果 Outcome', singular: '结果', icon: '✓', titleKey: 'title', description: '记录项目或行动产生的现实结果，并用证据比较预期与实际。', fields: [
    ...
    { key: 'validatedAt', label: '验证时间', type: 'datetime-local' }, { key: 'deliveredAt', label: '交付时间', type: 'datetime-local' }, { key: 'cycleStartedAt', label: '成果周期起点', type: 'datetime-local' },
    { key: 'valueClassification', label: '价值分类', type: 'select', options: [option('UNASSESSED', '未评估'), option('LOW', '低'), option('MEDIUM', '中'), option('HIGH', '高价值')] }, { key: 'valueConfirmedAt', label: '价值确认时间', type: 'datetime-local' },
    { key: 'nextAction', label: '下一步行动', multiline: true }, { key: 'failureReason', label: '失败 / 无效原因', multiline: true },
  ...
  { entity: 'deliverables', label: '成果资产', singular: '成果资产', icon: '▣', titleKey: 'title', description: '正式输出资产的元数据；复用 Notebook 文件，不复制原始文件。', fields: [
    ...
    { key: 'versionNumber', label: '版本号' }, { key: 'fileId', label: 'Notebook 文件', relation: 'notebookFiles' }, { key: 'notebookFolderId', label: '项目资料文件夹', relation: 'notebookFolders' }, { key: 'contextScope', label: '资料归属', type: 'select', options: [option('PROJECT', '项目'), option('COMPANY', '公司'), option('PERSONAL', '个人可复用')] }, { key: 'previousDeliverableId', label: '上一版本', relation: 'deliverables' }, { key: 'resultId', label: '所属结果', relation: 'results' }, { key: 'sourceResearchResultId', label: '来源调研结果', relation: 'researchResults' }, { key: 'projectId', label: '项目', relation: 'projects' }, { key: 'decisionId', label: '决策', relation: 'decisions' }, { key: 'workflowRunId', label: '工作链运行', relation: 'workflowRuns' }, { key: 'tags', label: '标签（逗号分隔）' }, { key: 'finalizedAt', label: '正式完成时间', type: 'datetime-local' },
  ] },
  ...
  { entity: 'capabilityPacks', label: '能力包', singular: '能力包', icon: '▦', titleKey: 'title', description: '能力资产的轻量清单；只引用标准资产与版本，不复制内容。', fields: [
    { key: 'title', label: '能力包名称' }, { key: 'summary', label: '适用边界', multiline: true }, { key: 'packType', label: '能力包类型', type: 'select', options: [option('ROLE', '岗位能力包'), option('MANAGEMENT', '管理能力包'), option('VENTURE', '创业经营能力包'), option('CUSTOM', '自定义能力包')] }, { key: 'status', label: '状态', type: 'select', options: [option('DRAFT', '草稿'), option('ACTIVE', '启用'), option('ARCHIVED', '已归档')] },
    { key: 'packVersion', label: '能力包版本' }, { key: 'domain', label: '适用领域' }, { key: 'audience', label: '适用角色 / 场景' }, { key: 'ownership', label: '归属说明' }, { key: 'portability', label: '可迁移性', type: 'select', options: [option('PERSONAL', '个人可迁移'), option('PUBLIC', '公开可复用'), option('COMPANY_SPECIFIC', '公司特定'), option('CONFIDENTIAL', '保密')] }, { key: 'sections', label: '分区（JSON）', multiline: true }, { key: 'tags', label: '标签（逗号分隔）' },
  ] },
  { entity: 'capabilityAssets', label: '能力资产', singular: '能力资产', icon: '✦', titleKey: 'title', description: '可重复利用的方法与工具；内容只保存在版本记录中。', fields: [
    { key: 'title', label: '资产名称' }, { key: 'summary', label: '摘要', multiline: true }, { key: 'assetKind', label: '资产类型', type: 'select', options: [option('PLAYBOOK', '方法手册'), option('SOP', 'SOP'), option('STANDARD', '标准'), option('CHECKLIST_TEMPLATE', '清单模板'), option('DOCUMENT_TEMPLATE', '文档模板'), option('TABLE_TEMPLATE', '表格模板'), option('PATTERN', '经验模式')] }, { key: 'status', label: '状态', type: 'select', options: [option('DRAFT', '草稿'), option('ACTIVE', '当前版本'), option('SUPERSEDED', '已替代'), option('ARCHIVED', '已归档')] },
    { key: 'domain', label: '适用领域' }, { key: 'maturity', label: '成熟度', type: 'select', options: [option('EXPERIMENTAL', '实验中'), option('TESTED', '已测试'), option('VALIDATED', '已验证'), option('RECOMMENDED', '推荐'), option('STANDARD', '标准'), option('REJECTED', '已拒绝')] }, { key: 'currentVersionId', label: '当前版本', relation: 'capabilityAssetVersions' }, { key: 'portability', label: '可迁移性', type: 'select', options: [option('PERSONAL', '个人可迁移'), option('PUBLIC', '公开可复用'), option('COMPANY_SPECIFIC', '公司特定'), option('CONFIDENTIAL', '保密')] }, { key: 'tags', label: '标签（逗号分隔）' },
  ] },
  { entity: 'capabilityAssetVersions', label: '能力资产版本', singular: '能力资产版本', icon: 'V', titleKey: 'title', description: '能力资产的不可静默覆盖版本；已发布版本只能被替代，不能直接改写。', fields: [
    { key: 'title', label: '版本名称' }, { key: 'capabilityAssetId', label: '能力资产', relation: 'capabilityAssets' }, { key: 'versionNumber', label: '版本号' }, { key: 'status', label: '版本状态', type: 'select', options: [option('DRAFT', '草稿'), option('ACTIVE', '已发布'), option('SUPERSEDED', '已被替代'), option('ARCHIVED', '已归档')] },
    { key: 'summary', label: '变更摘要', multiline: true }, { key: 'content', label: '内容 / 方法说明', multiline: true }, { key: 'structuredData', label: '结构化模板（JSON）', multiline: true }, { key: 'previousVersionId', label: '上一版本', relation: 'capabilityAssetVersions' }, { key: 'sourceResultId', label: '来源成果', relation: 'results' }, { key: 'sourceReviewId', label: '来源复盘', relation: 'reviews' }, { key: 'sourceInsightId', label: '来源洞见', relation: 'insights' }, { key: 'sourceKnowledgeId', label: '来源知识', relation: 'knowledge' }, { key: 'portability', label: '可迁移性', type: 'select', options: [option('PERSONAL', '个人可迁移'), option('PUBLIC', '公开可复用'), option('COMPANY_SPECIFIC', '公司特定'), option('CONFIDENTIAL', '保密')] }, { key: 'publishedAt', label: '发布时间', type: 'datetime-local' }, { key: 'promotedAt', label: '启用时间', type: 'datetime-local' },
  ] },
  ...
  { entity: 'packApplicationItems', label: '能力包应用项', singular: '应用项', icon: '›', titleKey: 'title', description: '记录某次项目应用使用的资产版本；与能力包定义分离。', fields: [
    { key: 'title', label: '名称' }, { key: 'packApplicationId', label: '能力包应用', relation: 'packApplications' }, { key: 'packItemId', label: '能力包引用', relation: 'capabilityPackItems' }, { key: 'capabilityAssetId', label: '能力资产', relation: 'capabilityAssets' }, { key: 'capabilityAssetVersionId', label: '资产版本', relation: 'capabilityAssetVersions' }, { key: 'templateInstanceId', label: '生成实例', relation: 'templateInstances' }, { key: 'executionTaskId', label: '执行任务', relation: 'tasks' }, { key: 'status', label: '状态', type: 'select', options: [option('APPLIED', '已应用'), option('SKIPPED', '已跳过'), option('COMPLETED', '已完成')] },
  ] },
  { entity: 'templateInstances', label: '模板实例', singular: '模板实例', icon: '▤', titleKey: 'title', description: '清单、文档和表格模板在项目中的实际实例；不回写模板版本。', fields: [
    { key: 'title', label: '实例名称' }, { key: 'instanceKind', label: '实例类型', type: 'select', options: [option('CHECKLIST', '清单'), option('DOCUMENT', '文档'), option('TABLE', '表格')] }, { key: 'sourceCapabilityAssetVersionId', label: '来源能力版本', relation: 'capabilityAssetVersions' }, { key: 'packApplicationId', label: '能力包应用', relation: 'packApplications' }, { key: 'projectId', label: '项目', relation: 'projects' }, { key: 'status', label: '状态', type: 'select', options: [option('DRAFT', '草稿'), option('IN_PROGRESS', '进行中'), option('COMPLETED', '已完成'), option('ARCHIVED', '已归档')] },
    { key: 'contentSnapshot', label: '文档内容', multiline: true }, { key: 'schemaSnapshot', label: '表格字段（JSON）', multiline: true }, { key: 'itemsSnapshot', label: '清单项（JSON）', multiline: true }, { key: 'rowsData', label: '表格行数据（JSON）', multiline: true },
  ] },
  { entity: 'capabilityImprovementProposals', label: '能力改进提案', singular: '能力改进提案', icon: '↑', titleKey: 'title', description: '由成果、复盘或洞见提出下一版能力草稿；AI 不会自动发布或验证。', fields: [
    { key: 'title', label: '提案名称' }, { key: 'capabilityAssetId', label: '能力资产', relation: 'capabilityAssets' }, { key: 'sourceType', label: '来源类型', type: 'select', options: [option('REVIEW', '复盘'), option('INSIGHT', '洞见'), option('EXPERIMENT', '实验'), option('RESULT', '成果'), option('USER', '用户'), option('AI_SUGGESTION', 'AI 建议')] }, { key: 'sourceCapabilityAssetVersionId', label: '来源版本', relation: 'capabilityAssetVersions' }, { key: 'sourceResultId', label: '来源成果', relation: 'results' }, { key: 'sourceReviewId', label: '来源复盘', relation: 'reviews' }, { key: 'sourceInsightId', label: '来源洞见', relation: 'insights' }, { key: 'problem', label: '观察到的问题', multiline: true }, { key: 'proposedChange', label: '拟议变更', multiline: true }, { key: 'expectedImpact', label: '预期影响', multiline: true }, { key: 'status', label: '状态', type: 'select', options: [option('DRAFT', '草稿'), option('PROPOSED', '待确认'), option('ACCEPTED', '已接受'), option('DEFERRED', '已延后'), option('REJECTED', '已拒绝'), option('IMPLEMENTED', '已形成新草稿')] }, { key: 'nextVersionId', label: '下一版本草稿', relation: 'capabilityAssetVersions' },
  ] },
  ...
  { entity: 'operationalLogs', label: '执行记录', singular: '执行记录', icon: '•', titleKey: 'content', description: '项目执行中发生的真实记录；先捕捉，再补充分类与关联。', fields: [
    ...
    { key: 'happenedAt', label: '发生时间', type: 'datetime-local' }, { key: 'projectId', label: '项目', relation: 'projects' }, { key: 'workflowRunId', label: '工作链运行', relation: 'workflowRuns' }, { key: 'taskId', label: '任务', relation: 'tasks' }, { key: 'personId', label: '人物', relation: 'people' }, { key: 'resultId', label: '结果', relation: 'results' }, { key: 'decisionId', label: '决策', relation: 'decisions' }, { key: 'experimentId', label: '实验', relation: 'experiments' }, { key: 'fileId', label: '附件', relation: 'notebookFiles' }, { key: 'metricName', label: '指标名称' }, { key: 'metricValue', label: '指标值' },
  ] },
  { entity: 'reviews', label: '复盘', singular: '复盘', icon: '◑', titleKey: 'title', description: '把现实转化为可以改变未来行动的经验。', fields: [
    ...
    { key: 'whatWorked', label: '哪些有效', multiline: true }, { key: 'whatFailed', label: '哪些无效', multiline: true }, { key: 'lesson', label: '学到了什么', multiline: true }, { key: 'decisionErrorClassification', label: '决策偏差确认', type: 'select', options: [option('NONE', '不是决策误判'), option('MISJUDGMENT', '确认误判'), option('WRONG_ASSUMPTION', '错误假设'), option('BIAS', '确认偏差')] },
    { key: 'doDifferently', label: '下次如何不同', multiline: true }, { key: 'nextAction', label: '下一步行动', multiline: true },
    { key: 'taskId', label: '任务', relation: 'tasks' }, { key: 'projectId', label: '项目', relation: 'projects' }, { key: 'goalId', label: '目标', relation: 'goals' }, { key: 'workflowRunId', label: '工作链运行', relation: 'workflowRuns' }, { key: 'templateInstanceId', label: '关联表格或清单', relation: 'templateInstances' }, { key: 'operationalLogIds', label: '执行记录', relation: 'operationalLogs', multiple: true },
  ...
  { entity: 'notebookCategories', label: 'Notebook 分类', singular: '分类', icon: '▦', titleKey: 'name', description: 'Notebook 内部的自定义分类；不是 EvoPilot Project。', fields: [
  ...
  { entity: 'decisions', label: '决策日志', singular: '决策', icon: '◆', titleKey: 'title', description: '记录问题、预测、结果与校准过程。', fields: [
    ...
    { key: 'evidence', label: '证据', multiline: true }, { key: 'assumptions', label: '关键假设', multiline: true }, { key: 'prediction', label: '预测', multiline: true }, { key: 'confidence', label: '置信度（0-100，兼容）', type: 'number' }, { key: 'confidenceAtDecision', label: '决策时置信度（0-100）', type: 'number' },
    { key: 'decisionLevel', label: '决策级别', type: 'select', options: [option('OPERATIONAL', '日常'), option('MATERIAL', '重要'), option('STRATEGIC', '战略')] },
    { key: 'importance', label: '重要性（人工确认）', type: 'select', options: [option('LOW', '低'), option('MEDIUM', '中'), option('HIGH', '高'), option('CRITICAL', '关键')] }, { key: 'riskLevel', label: '风险等级（人工确认）', type: 'select', options: [option('LOW', '低'), option('MEDIUM', '中'), option('HIGH', '高'), option('CRITICAL', '关键')] }, { key: 'reversibility', label: '可逆性', type: 'select', options: [option('REVERSIBLE', '可逆'), option('PARTIALLY_REVERSIBLE', '部分可逆'), option('IRREVERSIBLE', '不可逆')] },
    { key: 'evidenceAssessment', label: '证据充分度（人工确认）', type: 'select', options: [option('VERY_SUFFICIENT', '非常充分'), option('SUFFICIENT', '比较充分'), option('MODERATE', '一般'), option('INSUFFICIENT', '不足'), option('NONE', '几乎没有')] },
    { key: 'expectedOutcome', label: '预期结果', multiline: true }, { key: 'expectedImpact', label: '预期影响（定性）', multiline: true }, { key: 'successCriteria', label: '成功标准', multiline: true }, { key: 'expectedRevenueMinor', label: '预期收入', type: 'money', currencyKey: 'currency' }, { key: 'expectedCostMinor', label: '预期成本', type: 'money', currencyKey: 'currency' }, { key: 'currency', label: '币种', type: 'select', options: [option('CNY'), option('USD'), option('JPY'), option('EUR'), option('HKD')] },
    { key: 'expectedTimeMinutes', label: '预期时间（分钟）', type: 'number' }, { key: 'expectedCompletionDate', label: '预期完成日期', type: 'date' }, { key: 'reviewDueDate', label: '复盘日期（兼容）', type: 'date' }, { key: 'validationDueAt', label: '验证截止日期', type: 'date' },
    ...
    { key: 'date', label: '决策日期', type: 'date' }, { key: 'decisionAt', label: '做出选择时间', type: 'datetime-local' }, { key: 'choiceStatus', label: '选择状态', type: 'select', options: [option('PENDING', '待选择'), option('DECIDED', '已选择')] }, { key: 'executionStatus', label: '执行状态', type: 'select', options: [option('NOT_STARTED', '未开始'), option('IN_PROGRESS', '执行中'), option('EXECUTED', '已执行')] }, { key: 'executedAt', label: '执行完成时间', type: 'datetime-local' },
    { key: 'validationStatus', label: '校准结论（人工确认）', type: 'select', options: [option('PENDING', '待验证'), option('SUPPORTED', '得到支持'), option('PARTIALLY_SUPPORTED', '部分支持'), option('CONTRADICTED', '被现实反驳'), option('INCONCLUSIVE', '证据不足')] }, { key: 'validatedAt', label: '校准完成时间', type: 'datetime-local' }, { key: 'calibrationNotes', label: '校准备注', multiline: true },
    { key: 'status', label: '旧状态（兼容）', type: 'select', options: statuses.decision }, { key: 'outcome', label: '实际结果', multiline: true },
    ...
    { key: 'taskId', label: '任务', relation: 'tasks' }, { key: 'projectId', label: '项目', relation: 'projects' }, { key: 'goalId', label: '目标', relation: 'goals' }, { key: 'ownerId', label: '负责人', relation: 'people' },
  ...
  { entity: 'externalSources', label: '情报源', singular: '情报源', icon: '◉', titleKey: 'name', description: '定义 EvoPilot 应持续观察的关键词、账号、竞品和主题。', fields: [
  ...
  { entity: 'researchRequests', label: '调研需求', singular: '调研需求', icon: '⌕', titleKey: 'title', description: '在收纳箱中提出外部调研需求；确认前不会执行，也不会自动关联 EvoPilot。', fields: [
  ...
  { entity: 'researchResults', label: '调研结果', singular: '调研结果', icon: '◈', titleKey: 'title', description: '外部调研的结果与证据；由用户决定是否分类、存入知识或关联 EvoPilot。', fields: [
