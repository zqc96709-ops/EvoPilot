...
const editBody = (content) => execute(`const editor=document.querySelector('.notebook-rich-editor');editor.textContent=arguments[0];editor.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:arguments[0]}));return true`, [content])
...
try {
  ...
  console.log(JSON.stringify({ status: 'passed', noteId: note.id, checks: ['600ms idle autosave', 'category flush preserves 300+ chars', 'note switch flush', 'SIGKILL recovery', 'recovery promoted to SQLite', 'one pending note UPDATE'] }))
} catch (error) {
