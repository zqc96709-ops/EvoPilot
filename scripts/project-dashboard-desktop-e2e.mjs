...
try {
  ...
  await responsive('capability asset filter', () => execute(`const select=document.querySelector('.capability-picker-filters select');const setter=Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set;setter.call(select,'CHECKLIST_TEMPLATE');select.dispatchEvent(new Event('change',{bubbles:true}));return select.value`), () => execute('return document.querySelector(".capability-picker-filters select")?.value === "CHECKLIST_TEMPLATE"'))
  await responsive('capability preview', () => click('.project-capability-picker .button', '预览'), () => execute('return document.querySelector(".capability-apply-preview")?.innerText.includes("项目检查清单") || Boolean(document.querySelector(".capability-apply-preview"))'))
  await responsive('capability quick use', () => click('.capability-apply-preview .primary', '确认应用'), () => execute('return !document.querySelector(".project-capability-picker")'))
  ...
  await execute(`const input=document.querySelector('.workchain-panel .quick-log-input textarea');const setter=Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,'value').set;setter.call(input,'真实 UI Run 记录');input.dispatchEvent(new Event('input',{bubbles:true}));return input.value`)
  await responsive('workflow run auto-linked log', () => click('.workchain-panel .quick-log-input .primary', '保存'), () => execute('return document.querySelector(".workchain-panel")?.innerText.includes("真实 UI Run 记录")'))
  ...
  await execute(`const input=document.querySelector('.material-folder-toolbar input');const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;setter.call(input,'项目资料');input.dispatchEvent(new Event('input',{bubbles:true}));return input.value`)
  await responsive('materials folder create', () => click('.material-folder-toolbar .button', '新建文件夹'), () => execute('return [...document.querySelectorAll(".material-folder-toolbar option")].some((item)=>item.textContent.includes("项目资料"))'))
  await responsive('materials move reference', () => execute(`const select=document.querySelector('.material-list article select');const target=[...select.options].find((item)=>item.textContent.includes('项目资料'));const setter=Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set;setter.call(select,target.value);select.dispatchEvent(new Event('change',{bubbles:true}));return select.value`), () => execute('return document.querySelector(".material-list article select")?.selectedOptions[0]?.textContent.includes("项目资料")'))
  ...
  console.log(JSON.stringify({ status: 'passed', screenshotPath, responseMs, checks: ['real project dashboard fixture', 'period change', 'five-tab project workbench', 'header quick capture', 'capability asset filter', 'capability preview', 'capability quick use', 'operating state save', 'operational quick capture', 'workflow run auto-linked log', 'structured table save', 'materials link visibility', 'materials folder create', 'materials move reference', 'review tab', 'UI project creation updates dashboard', '1536 desktop screenshot'] }))
} catch (error) {
