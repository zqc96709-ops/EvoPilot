...
try {
  ...
  const funnel = await execute('const panel=document.querySelector(".di-funnel").getBoundingClientRect();const stages=[...document.querySelectorAll(".di-funnel button")].map((button)=>({label:button.innerText,rect:button.getBoundingClientRect().toJSON()}));return {panel,stages}')
  if (funnel.stages.length !== 5 || funnel.stages.some((stage) => stage.rect.top < funnel.panel.top || stage.rect.bottom > funnel.panel.bottom)) throw new Error(`Decision calibration lifecycle is clipped: ${JSON.stringify(funnel)}`)
